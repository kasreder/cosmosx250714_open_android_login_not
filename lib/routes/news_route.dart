...
/// ✅ **뉴스 & 공지사항 브랜치**
StatefulShellBranch buildNewsBranch() {
...
GoRoute _buildBoardRoute({
  ...
  return GoRoute(
    path: middleMunu,
    pageBuilder: (context, state) {
      print('news_routes.dart: /$topMunu/$middleMunu 접근');
    ...
    routes: [
      GoRoute(
        ...
        builder: (context, state) {
          ...
          if (id == null) {
            print('❌ news_routes.dart: /$topMunu/$middleMunu/:itemIndex 값이 없음!');
            return const Scaffold(
              body: Center(child: Text("잘못된 접근입니다.")),
            );
          }

          print('✅ news_routes.dart: /$topMunu/$middleMunu/$id 접근 (fullUrl: $splitUrl)');

          return FutureBuilder<Map<String, dynamic>>(
            future: extractMenuMapData1(splitUrl, id), // 서버에서 데이터 로드
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                print('🔄 news_routes.dart: /nwn/$middleMunu/$id 데이터 로드 중...');
                return const Scaffold(
              ...
              if (snapshot.hasError) {
                print('❌ news_routes.dart: /$topMunu/$middleMunu/$id 에러 발생: ${snapshot.error}');
                return Scaffold(
                  body: Center(
              ...
              print('✅ news_routes.dart: /$topMunu/$middleMunu/$id 데이터 로드 완료 (currentIndex: $currentIndex)');
        ...
        routes: [
          GoRoute(
            path: 'update',
            builder: (context, state) {
              print('🛠 news_routes.dart: /$topMunu/$middleMunu/:itemIndex/update 접근');
