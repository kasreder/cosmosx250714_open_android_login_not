...
import 'nwn_notice_details_screen.dart';

// ignore: camel_case_types
class NoticebuildCommentList extends StatefulWidget {
  final Function(bool) updateVisibility;
  final List<CommentP> commentData;
  final String? itemIndex;

  const NoticebuildCommentList({
  ...
  @override
  State<NoticebuildCommentList> createState() => NoticebuildCommentListState();
}

class NoticebuildCommentListState extends State<NoticebuildCommentList> {
  final FocusNode NoticecommentFocusNode = FocusNode();
  final Map<int, FocusNode> NoticereplyFocusNodes = {};
  final TextEditingController NoticenewCommentController =
  TextEditingController(); // 새 댓글을 위한 컨트롤러
  final Map<int, TextEditingController> NoticereplyControllers = {};
  final Map<int, bool> NoticeshowReplyField = {};
  // final quill.QuillController NoticenewCommentController = quill.QuillController.basic();

  @override
  void initState() {
    super.initState();
    NoticecommentFocusNode.addListener(_handleFocusChange);
  }

  @override
  void dispose() {
    NoticecommentFocusNode.removeListener(_handleFocusChange);
    NoticecommentFocusNode.dispose();
    NoticereplyFocusNodes.forEach((key, focusNode) {
  ...
  void _handleFocusChange() {
    if (NoticecommentFocusNode.hasFocus ||
        NoticereplyFocusNodes.values.any((focusNode) => focusNode.hasFocus)) {
              ...
              child: Column(
                children: [
                  SizedBox(
                    width: ResponsiveWidth.getResponsiveWidth(context),
                    child: TextFormField(
                      focusNode: NoticecommentFocusNode,
                      controller: NoticenewCommentController,
                      decoration: InputDecoration(
                        ...
                        suffixIcon: IconButton(
                          ...
                          onPressed: () {
                            dynamic text = NoticenewCommentController.text
                                .trim();
                            if (text.isNotEmpty) {
                              _sendComment(text);
                              NoticenewCommentController.clear();
                            } else {
  ...
  Widget buildComment(CommentP comment) {
    ...
    NoticereplyFocusNodes[comment.comment_id] ??= FocusNode()
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          setState(() {
                            ...
                            !(NoticeshowReplyField[comment.comment_id] ??
                                false);
                            NoticeshowReplyField.clear();
                            NoticeshowReplyField[comment.comment_id] =
                                currentFieldState;
                          });
                        },
                ...
                if (NoticeshowReplyField[comment.comment_id] ?? false)
                  Padding(
                    ...
                    child: Align(
                      ...
                      child: SizedBox(
                        ...
                        child: TextFormField(
                          focusNode: NoticereplyFocusNodes[comment.comment_id],
                          controller: NoticereplyControllers[comment
                              .comment_id] ??=
                          ...
                          decoration: InputDecoration(
                            ...
                            suffixIcon: IconButton(
                              ...
                              onPressed: () {
                                String text =
                                    NoticereplyControllers[comment.comment_id]
                                        ?.text
                                        .trim() ??
                                        '';
