...
import 'nwn_news_comment_list_screen.dart';
...
class NewsDetailsScreenState extends State<NewsDetailsScreen> {
  ...
  int? currentIndex;
  List<dynamic>? mappedIds;
  Map<String, dynamic>? extraData; // extraData 필드 추가
  final String middleMunu = 'nwn/news';
  ...
  String constructUrl() {
    // print('디테일 extraData : ${widget.extraData}');
    ...
    return "https://api.cosmosx.co.kr/news/$itemIndex";
  }
  ...
  Future<void> extractMenuMapData2(String? itemIndex) async {
    ...
    try {
      ...
      final url = "https://api.cosmosx.co.kr/news/$itemIndex";
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // 필요하면 key도 추가 설정 (랜덤 생성 또는 특정 값 사용)
  ...
  Widget build(BuildContext context) {
    ...
    final mappedCount = mappedIds?.length ?? 0;
    ...
    return Scaffold(
      ...
      body: FutureBuilder<PostWithComments>(
          ...
          builder: (context, snapshot) {
            ...
            print('NewsData11: ${jsonEncode(NewsData.toJson())}');
                              ...
                              child: Row(
                                ...
                                children: [
                                  TextButton(
                                    ...
                                    child: const Text('<', style: const TextStyle(fontSize: 15)),
                                  ),
                                  TextButton(
                                    onPressed: () {
                                      ...
                                      SizedBox(
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            SelectableText(NewsData.nickname ?? 'No nickname', style: const TextStyle(color: Colors.grey, fontSize: 15)),
                                            SelectableText(
                                              '조회 ${(NewsData.views ?? 0).toString()}',
                                              style: Theme.of(context).textTheme.bodyLarge!.copyWith(color: Colors.grey, fontSize: 15),
                                            ),
                                            Text(DateUtil.formatDate(NewsData.created_at), style: const TextStyle(color: Colors.grey, fontSize: 15)),
                                          ],
                                        ),
                                      ),
                                        ...
                                        child: Row(
                                          ...
                                          children: <Widget>[
                                            ...
                                            HeroMode(
                                              ...
                                              child: TextButton(
                                                onPressed: () {
                                                  print('이전글 extraData: ${widget.extraData}');
                                                  final extraData = widget.extraData; // extraData를 가져옴
                                                  print('이전글 extraData length 11111: ${widget.extraData?.length}');
                                                  final currentIndex = extraData?['currentIndex'] as int? ?? 0; // 현재 인덱스
                                                  print('이전글 currentIndex 뒤에서 몇번째 값인가 22222: $currentIndex');
                                                  final mappedIds = extraData?['mappedIds'] as List<int>? ?? []; // ID 목록
                                                  print('이전글 mappedIds 데이터 갯수1 33333: ${mappedIds.length}');

                                                  if (currentIndex < mappedIds.length) {
                                                    // 이전 글로 이동
                                                    final updatedIndex = mappedIds.length - 1 - (currentIndex); // 뒤에서 가져오기
                                                    print('이전글 updatedIndex 앞에서 몇번째 순번 인가0 44444: $updatedIndex');
                                                    final newItemIndex = mappedIds[updatedIndex - 1]; // 새로운 itemIndex 값 가져오기
                                                    print('이전글 newItemIndex 앞에서 순번의 값인가 55555: $newItemIndex');
                                                    // 업데이트된 데이터를 준비
                                                    ...
                                                    print('이전글 extraData: ${widget.extraData}');
                                                    print('이전글 updatedExtraData: $extraData');

                                                    // 새로운 경로로 이동
                                                    context.go(
                                                      '/$middleMunu/$newItemIndex',
                                                      extra: ValueKey(extraData),
                                                    );
                                                    print('이전글updatedExtraData1: $extraData');
                                                  }
                                                },
                                            ...
                                            TextButton(
                                              onPressed: () {
                                                // Navigator.of(context).pop(); // 단순히 pop하고 값을 전달
                                                // 현재 데이터를 가져옴
                                                print('다음글 extraData: $widget.extraData');
                                                final extraData = widget.extraData; // extraData를 가져옴
                                                final currentIndex = extraData?['currentIndex'] as int? ?? 0; // 현재 인덱스
                                                final mappedIds = extraData?['mappedIds'] as List<int>? ?? []; // ID 목록
                                                ...
                                                if (currentIndex > 0) {
                                                  ...
                                                  print('다음글updatedExtraData: $extraData');
                                                }
                                              },
