...
import 'nwn_news_details_screen.dart';

// ignore: camel_case_types
class NewsbuildCommentList extends StatefulWidget {
  final Function(bool) updateVisibility;
  final List<CommentP> commentData;
  final String? itemIndex;

  const NewsbuildCommentList({
  ...
  @override
  State<NewsbuildCommentList> createState() => NewsbuildCommentListState();
}

class NewsbuildCommentListState extends State<NewsbuildCommentList> {
  final FocusNode NewscommentFocusNode = FocusNode();
  final Map<int, FocusNode> NewsreplyFocusNodes = {};
  final TextEditingController NewsnewCommentController =
  TextEditingController(); // 새 댓글을 위한 컨트롤러
  final Map<int, TextEditingController> NewsreplyControllers = {};
  final Map<int, bool> NewsshowReplyField = {};
  // final quill.QuillController NewsnewCommentController = quill.QuillController.basic();

  @override
  void initState() {
    super.initState();
    NewscommentFocusNode.addListener(_handleFocusChange);
  }

  @override
  void dispose() {
    NewscommentFocusNode.removeListener(_handleFocusChange);
    NewscommentFocusNode.dispose();
    NewsreplyFocusNodes.forEach((key, focusNode) {
  ...
  void _handleFocusChange() {
    if (NewscommentFocusNode.hasFocus ||
        NewsreplyFocusNodes.values.any((focusNode) => focusNode.hasFocus)) {
              ...
              child: Column(
                children: [
                  SizedBox(
                    width: ResponsiveWidth.getResponsiveWidth(context),
                    child: TextFormField(
                      focusNode: NewscommentFocusNode,
                      controller: NewsnewCommentController,
                      decoration: InputDecoration(
                        ...
                        suffixIcon: IconButton(
                          ...
                          onPressed: () {
                            dynamic text = NewsnewCommentController.text
                                .trim();
                            if (text.isNotEmpty) {
                              _sendComment(text);
                              NewsnewCommentController.clear();
                            } else {
  ...
  Widget buildComment(CommentP comment) {
    ...
    NewsreplyFocusNodes[comment.comment_id] ??= FocusNode()
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          setState(() {
                            ...
                            !(NewsshowReplyField[comment.comment_id] ??
                                false);
                            NewsshowReplyField.clear();
                            NewsshowReplyField[comment.comment_id] =
                                currentFieldState;
                          });
                        },
                ...
                if (NewsshowReplyField[comment.comment_id] ?? false)
                  Padding(
                    ...
                    child: Align(
                      ...
                      child: SizedBox(
                        ...
                        child: TextFormField(
                          focusNode: NewsreplyFocusNodes[comment.comment_id],
                          controller: NewsreplyControllers[comment
                              .comment_id] ??=
                          ...
                          decoration: InputDecoration(
                            ...
                            suffixIcon: IconButton(
                              ...
                              onPressed: () {
                                String text =
                                    NewsreplyControllers[comment.comment_id]
                                        ?.text
                                        .trim() ??
                                        '';
