...
class NewsListScreenState extends State<NewsListScreen> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      Provider.of<ViewCountProvider>(context, listen: false).fetchPostDataFromAPI("news");
    });
  }

  final String url = "https://api.cosmosx.co.kr/news"; // 데이터 API URL
  ...
  Widget buildNewsPageIndex(int totalItems) {
...
//   /// 하단 페이지 번호 출력 위젯
//   Row buildNewsPageIndex(int startPage, int endPage) {
//     return Row(
//       mainAxisAlignment: MainAxisAlignment.center,
//       children: [
//         IconButton(
...
//
//                   totalItems = NewsListData.length;
//                   // 동적 페이지 번호 계산
...
//                       SizedBox(width: ResponsiveWidth.getResponsiveWidth(context), child: const Divider(color: Colors.black54, thickness: 0.3)),
//                       buildNewsExpanded(deviceWidth, NewsListData),
//                       SizedBox(width: ResponsiveWidth.getResponsiveWidth(context), child: const Divider(color: Colors.black54, thickness: 0.3)),
...
//
//   /// 게시판 리스트 출력 위젯
//   Expanded buildNewsExpanded(double deviceWidth, List<BoardP> NewsListData) {
//     //Create a list that maps index to id
//     // final mappedIds = NewsListData
//     //     .asMap()
//     //     .entries
//     //     .map((entry) => entry.value.id)
//     //     .toList();
//     // ID 목록 생성
//     final mappedIds = NewsListData.map((e) => e.id).toList();
//     final viewCountProvider = Provider.of<ViewCountProvider>(context);
//
...
//             itemCount: min(
//               itemsPerPage,
//               NewsListData.length - currentPage * itemsPerPage,
//             ),
//             itemBuilder: (BuildContext context, int index) {
//               int itemIndex = (NewsListData.length - 1) - (currentPage * itemsPerPage + index); // 내림차순으로 항목 계산
//               int totalItemsCount = NewsListData.length; // 내림차순으로 News로 검색된 항목 계산
//               int listOrderNumber = totalItemsCount - (currentPage * itemsPerPage + index);
//               final item = NewsListData[index];
...
//                   };
//                   // 상세 페이지 이동
//                   String newPath = '${widget.detailPath}${NewsListData[itemIndex].id}';
//                   context.go(
//                     newPath,
...
//                   print('NewsListData[itemIndex].id : ${NewsListData[itemIndex].id}');
//                 },
//                 leading: SelectableText(
//                   '$listOrderNumber', // 항목 번호
//                   style: Theme.of(context).textTheme.titleSmall,
...
//                   children: [
//                     Expanded(
//                         child: Text("${NewsListData[itemIndex].title} ",
//                             overflow: TextOverflow.ellipsis, maxLines: 1, softWrap: false,
//                             style: Theme.of(context).textTheme.titleMedium)),
//                     const SizedBox(width: 5), // 제목과 조회수 간격 조절
//                     SelectableText(
...
//                     ),
//                     SelectableText(
//                       (NewsListData[itemIndex].views ?? 0).toString(),
//                       style: Theme.of(context).textTheme.bodyLarge!.copyWith(color: const Color(0xff4C6EF5), fontSize: 12),
//                     ),
//                   ],
//                 ),
...
//                   children: <Widget>[
//                     SelectableText(
//                       NewsListData[itemIndex].nickname,
//                       style: Theme.of(context).textTheme.labelSmall!.copyWith(color: Colors.black54, fontSize: 10),
//                     ),
//                     SelectableText(
//                       '     ${DateUtil.formatDate(NewsListData[itemIndex].created_at)}',
//                       style: Theme.of(context).textTheme.titleSmall!.copyWith(
//                             color: Colors.black54,
//                           ),
//                     ),
...
//               //                           SelectableText(
//               //                             DateUtil.formatDate(
//               //                                 NewsListData[itemIndex].created_at),
...
//
// class NewsListScreen extends StatefulWidget {
//   const NewsListScreen({super.key, required String label, required String detailPath});
//
//   @override
...
//             onTap: () {
//               // 상세 페이지로 이동
//               Navigator.pushNamed(context, '/comm1/news/$postId');
//             },
//           );
//         },
//       ),
