...
import 'comm_Free_details_screen.dart';

// ignore: camel_case_types
class FreebuildCommentList extends StatefulWidget {
  final Function(bool) updateVisibility;
  final List<CommentP> commentData;
  final String? itemIndex;

  const FreebuildCommentList({
  ...
  @override
  State<FreebuildCommentList> createState() => FreebuildCommentListState();
}

class FreebuildCommentListState extends State<FreebuildCommentList> {
  final FocusNode FreecommentFocusNode = FocusNode();
  final Map<int, FocusNode> FreereplyFocusNodes = {};
  final TextEditingController FreenewCommentController =
  TextEditingController(); // 새 댓글을 위한 컨트롤러
  final Map<int, TextEditingController> FreereplyControllers = {};
  final Map<int, bool> FreeshowReplyField = {};
  // final quill.QuillController FreenewCommentController = quill.QuillController.basic();

  @override
  void initState() {
    super.initState();
    FreecommentFocusNode.addListener(_handleFocusChange);
  }

  @override
  void dispose() {
    FreecommentFocusNode.removeListener(_handleFocusChange);
    FreecommentFocusNode.dispose();
    FreereplyFocusNodes.forEach((key, focusNode) {
  ...
  void _handleFocusChange() {
    if (FreecommentFocusNode.hasFocus ||
        FreereplyFocusNodes.values.any((focusNode) => focusNode.hasFocus)) {
  ...
  String constructUrl() {
    ...
    // return "https://terraforming.info/Free/$itemIndex";
                ...
                children: [
                  SizedBox(
                    ...
                    child: TextFormField(
                      ...
                      controller: FreenewCommentController,
                      decoration: InputDecoration(
                        ...
                        suffixIcon: IconButton(
                          ...
                          onPressed: () {
                            dynamic text = FreenewCommentController.text
                                .trim();
                            if (text.isNotEmpty) {
                              _sendComment(text);
                              FreenewCommentController.clear();
                            } else {
  ...
  Widget buildComment(CommentP comment) {
    ...
    FreereplyFocusNodes[comment.comment_id] ??= FocusNode()
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            DateUtil.formatDateCommentIcon(
                              comment.comment_created_at,
                              iconSize: 12, // 아이콘 크기 변경
                              textSize: 10, // 텍스트 크기 변경
                              textColor: Colors.grey, // 텍스트 색상 변경
                            ),
                          ],
                        ),
                      ),
                      IconButton(
                        ...
                        onPressed: () {
                          setState(() {
                            ...
                            !(FreeshowReplyField[comment.comment_id] ??
                                false);
                            FreeshowReplyField.clear();
                            FreeshowReplyField[comment.comment_id] =
                                currentFieldState;
                          });
                        },
                ...
                if (FreeshowReplyField[comment.comment_id] ?? false)
                  Padding(
                    ...
                    child: Align(
                      ...
                      child: SizedBox(
                        ...
                        child: TextFormField(
                          focusNode: FreereplyFocusNodes[comment.comment_id],
                          controller: FreereplyControllers[comment
                              .comment_id] ??=
                          ...
                          decoration: InputDecoration(
                            ...
                            suffixIcon: IconButton(
                              ...
                              onPressed: () {
                                String text =
                                    FreereplyControllers[comment.comment_id]
                                        ?.text
                                        .trim() ??
                                        '';
