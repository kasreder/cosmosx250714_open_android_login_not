...
class FreeListScreen extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => FreeListScreenState();
}

class FreeListScreenState extends State<FreeListScreen> {
  final int itemsPerPage = 15; // 한 페이지당 표시할 항목 수
  int totalItems = 0; // 전체 항목 수
  int currentPage = 0; // 현재 페이지 번호
  String baordname = "떠들어 봅시다";
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      Provider.of<ViewCountProvider>(context, listen: false).fetchPostDataFromAPI("free");
    });
  }

  final String url = "https://api.cosmosx.co.kr/free"; // 데이터 API URL
  ...
  Widget buildFreePageIndex(int totalItems) {
...
//   /// 하단 페이지 번호 출력 위젯
//   Row buildFreePageIndex(int startPage, int endPage) {
//     return Row(
//       mainAxisAlignment: MainAxisAlignment.center,
//       children: [
//         IconButton(
...
//                   final FreeListData = snapshot.data!;
//
//                   totalItems = FreeListData.length;
//                   // 동적 페이지 번호 계산
...
//                       SizedBox(width: ResponsiveWidth.getResponsiveWidth(context), child: const Divider(color: Colors.black54, thickness: 0.3)),
//                       buildFreeExpanded(deviceWidth, FreeListData),
//                       SizedBox(width: ResponsiveWidth.getResponsiveWidth(context), child: const Divider(color: Colors.black54, thickness: 0.3)),
//                       buildFreeRow(startPage, endPage),
//                     ],
//                   );
//                 }
...
//
//   /// 게시판 리스트 출력 위젯
//   Expanded buildFreeExpanded(double deviceWidth, List<BoardP> FreeListData) {
//     //Create a list that maps index to id
//     // final mappedIds = FreeListData
//     //     .asMap()
//     //     .entries
//     //     .map((entry) => entry.value.id)
//     //     .toList();
//     // ID 목록 생성
//     final mappedIds = FreeListData.map((e) => e.id).toList();
//     final viewCountProvider = Provider.of<ViewCountProvider>(context);
//
...
//             itemCount: min(
//               itemsPerPage,
//               FreeListData.length - currentPage * itemsPerPage,
//             ),
//             itemBuilder: (BuildContext context, int index) {
//               int itemIndex = (FreeListData.length - 1) - (currentPage * itemsPerPage + index); // 내림차순으로 항목 계산
//               int totalItemsCount = FreeListData.length; // 내림차순으로 Free로 검색된 항목 계산
//               int listOrderNumber = totalItemsCount - (currentPage * itemsPerPage + index);
//               final item = FreeListData[index];
...
//                   };
//                   // 상세 페이지 이동
//                   String newPath = '${widget.detailPath}${FreeListData[itemIndex].id}';
//                   context.go(
//                     newPath,
...
//                   print('FreeListData[itemIndex].id : ${FreeListData[itemIndex].id}');
//                 },
//                 leading: SelectableText(
//                   '$listOrderNumber', // 항목 번호
//                   style: Theme.of(context).textTheme.titleSmall,
...
//                   children: [
//                     Expanded(
//                         child: Text("${FreeListData[itemIndex].title} ",
//                             overflow: TextOverflow.ellipsis, maxLines: 1, softWrap: false,
//                             style: Theme.of(context).textTheme.titleMedium)),
//                     const SizedBox(width: 5), // 제목과 조회수 간격 조절
//                     SelectableText(
...
//                     ),
//                     SelectableText(
//                       (FreeListData[itemIndex].views ?? 0).toString(),
//                       style: Theme.of(context).textTheme.bodyLarge!.copyWith(color: const Color(0xff4C6EF5), fontSize: 12),
//                     ),
//                   ],
//                 ),
...
//                   children: <Widget>[
//                     SelectableText(
//                       FreeListData[itemIndex].nickname,
//                       style: Theme.of(context).textTheme.labelSmall!.copyWith(color: Colors.black54, fontSize: 10),
//                     ),
//                     SelectableText(
//                       '     ${DateUtil.formatDate(FreeListData[itemIndex].created_at)}',
//                       style: Theme.of(context).textTheme.titleSmall!.copyWith(
//                             color: Colors.black54,
//                           ),
//                     ),
...
//               //                           SelectableText(
//               //                             DateUtil.formatDate(
//               //                                 FreeListData[itemIndex].created_at),
...
//
//   /// 하단 페이지 번호 출력 위젯
//   Row buildFreeRow(int startPage, int endPage) {
//     return Row(
//       mainAxisAlignment: MainAxisAlignment.center,
//       children: [
//         IconButton(
...
//
// class FreeListScreen extends StatefulWidget {
//   const FreeListScreen({super.key, required String label, required String detailPath});
//
//   @override
...
//             onTap: () {
//               // 상세 페이지로 이동
//               Navigator.pushNamed(context, '/comm1/free/$postId');
//             },
//           );
//         },
//       ),
